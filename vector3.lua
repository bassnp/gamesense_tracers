...
function vector_c:length(selfX, selfY, selfZ)
	return math.sqrt((selfX * selfX) + (selfY * selfY) + (selfZ * selfZ))
end

function vector_c:distance(destination, selfX, selfY, selfZ)
	return (vector(destination.x - selfX, destination.y - selfY, destination.z - selfZ)):length(selfX, selfY, selfZ)
end


function vector_c:normalize(selfX, selfY, selfZ)
	local length = self:length(selfX, selfY, selfZ)
	if (length ~= 0) then
		self.x = selfX / length
		self.y = selfY / length
		self.z = selfZ / length
